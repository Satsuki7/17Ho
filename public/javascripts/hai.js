...
function createYamahai(){            
    let shuffleArray = (arr) => {                   //配列ランダムソート(シャッフル)関数
        let n = arr.length;                         //変数n＝配列の個数
        let temp = 0, i = 0;                        //変数temp,i＝初期値0
        while (n) {                                 //nが0になるまでwhileを繰り返す
    ...
    //山牌返却
    return shuffleArray([...yamahaiGenerator(KINDS.SUUPAI,SUUPAI_VALUE)             //上のyamahaiGeneretor()の引数として使われ、結果136枚すべての牌が生成される
